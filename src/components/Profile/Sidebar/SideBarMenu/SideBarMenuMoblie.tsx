...
const SideBarMenuMoblie = memo(
  ({ openMenu, menu_active, subMenu_active, handleOpenMenu }: any) => {
    return (
      <div
        className={`profile__navMobile ${
          openMenu && "profile__navMobile__open"
        }`}
      ...
      >
        {menu.map((item, index) => (
          <div
            key={index}
            className="profile__navMobile__menuMobile"
        ...
        <AiOutlineClose
          size={26}
          className="profile__navMobile__btn"
          onClick={handleOpenMenu}
        />
      </div>
    );
